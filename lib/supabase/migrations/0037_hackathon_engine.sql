-- ============================================================================
-- PROPHEZY — 0037_hackathon_engine.sql
-- Purpose : Promoted from lib/hackathons/supabase/migrations/0001_hackathon_engine_tables.sql
--           into the real migration sequence, with one change: the
--           original file's `saved_hackathons` table is DROPPED from this
--           version — it's redundant with hackathon_registrations.status =
--           'saved' (TrackingRepository, the only thing any hackathon
--           service code actually reads/writes, never references a
--           separate saved_hackathons table at all). Keeping both would
--           mean "save" writes to one table while every read path checks
--           another. Everything else is unchanged from the original.
-- Depends : 0001_extensions.sql (vector)
-- ============================================================================
...
-- Tracking entries: status (saved/registered/in_progress/submitted/
-- completed/withdrawn) + prep/submission progress per user per hackathon.
-- "Save/unsave" and "bookmarking" both map to status = 'saved' here.
...
-- Notifications queued for a user (deadline/registration/submission
-- reminders, theme/technology match alerts, new-hackathon alerts).
...
-- Vector embeddings for semantic search (not wired this pass — see
-- lib/hackathons/README's SearchService section; keyword search is the
-- active path today. Table created so it's ready when vector search is).
